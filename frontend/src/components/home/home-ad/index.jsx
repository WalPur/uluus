...
const HomeAd = () => {
	...
	useEffect(() => {
		...
		const api = input || uluusId.length ? `https://uluus.ru/${input === '' ? 'api/' : 'search/'}${category}/${input ? input + '/' : ''}?limit=${advertCount}&offset=${(page - 1) * advertCount}&uluus=${uluusId.join(',')}` : `https://uluus.ru/api/${category}/?limit=${advertCount}&offset=${(page - 1) * advertCount}`;
		axios
			.get(api)
			.then((response) => {
